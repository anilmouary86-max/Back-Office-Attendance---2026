...
function setupSheets_() {
  ...
  const headers = ["PIN", "Employee Code", "Employee Name", "Designation", "Date", "Status", "Approval", "Saved At", "Approved At"];
...
function save_(pin, date, status) {
  ...
  const approval = status === "A" ? "Approved" : "Pending";
  const row = [employee.pin, employee.code, employee.name, employee.department, date, status, approval, new Date(), ""];
  if (updateRow > 0) sheet.getRange(updateRow, 1, 1, row.length).setValues([row]);
  else sheet.appendRow(row);

  const approvalText = approval === "Pending" ? "Pending for CMO approval" : "Approved";
  return { ok: true, employee: publicEmployee_(employee), date, status, approval, message: approvalText };
}
...
function review_(adminPin, targetPin, date, decision) {
  ...
  for (let i = 1; i < values.length; i++) {
    if (String(values[i][0]) === employee.pin && formatDate_(values[i][4]) === date) {
      ...
      if (!["WFH", "CL"].includes(status)) return { ok: false, error: "Sirf WFH/CL approval chahiye." };
      sheet.getRange(i + 1, 7).setValue(decision);
      sheet.getRange(i + 1, 9).setValue(new Date());
      return { ok: true, message: `${employee.name} ${date} ${decision}.` };
    }
  }
  return { ok: false, error: "Pending attendance record nahi mila." };
}
...
function admin_(pin) {
  ...
  data.forEach((row) => {
    ...
    const approval = String(row[6] || (status === "A" ? "Approved" : "Pending"));
    if (grouped[pinValue] && date) {
      grouped[pinValue].days[date] = { status, approval };
    }
  });
